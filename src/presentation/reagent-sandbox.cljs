...
(defcard-rg example-component
 ...
 Make sure the anonymous function component is the last statement in the editor."
 (fn [data-atom _] [component-block data-atom])
 (atom {:input
"(defn reset-state [state]
   (reset! state {:count 0}))

(defn update-count-button [state func label]
 [:button.btn.btn-primary
  {:on-click #(swap! state update :count func)} label])
